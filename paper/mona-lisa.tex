...
\end{equation}
\noindent 
to be the ``realised AC density'' in cell $j$, and the limit of this function as $\int_{\mathcal{C}_j}$ shrinks to the point $\bm{s}$, to be the realised AC density surface at $\bm{s}$:
\begin{equation}
...
\begin{equation}
\widehat{D}_r(\bm{s}) = \sum_{i = 1}^n \hat{f}_i(\bm{s}) + (\widehat{N} - n)f_u(\bm{s})
\end{equation}
