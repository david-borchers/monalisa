...
\subsection{Realised AC density surface} \label{s:racd}
 
The second kind of density we call the ``realised AC density''. Suppose that we divide $\mathcal{S}$ into a grid of cells, the $j$th of which comprises the points in the set $\mathcal{C}_j$, and each has area $a$.
Suppose also that we don't observe the ACs, but we treat them as random variables, each with its own distribution and we have a probability density function (PDFs) for each AC: $f_1(\bm{s}),\ldots,f_N(\bm{s})$. We define random variables $\delta_{1j},\ldots,\delta_{N_j}$ ($j=1,\ldots,N$), such that the $\delta_{ij}=1$ if the $i$th AC is in $\mathcal{C}_j$ and $\delta_{ij}=0$ otherwise. The expected value of $\delta_{ij}$ is then 
\begin{equation}
...
\begin{equation}
  D_{rj} = \sum_{i = 1}^N E(\delta_{ij})/a. \label{eq:realised-D_rj}
\end{equation}
\noindent 
...
\end{equation}
\noindent
Given estimates for the number of ACs in the survey region, $\hat{N}$, and for the PDFs of their locations, $\hat{f}_i(\bm{s}),\ldots,\hat{f}_{\hat{N}}(\bm{s})$, we can estimate the realised AC density surface as $\widehat{D}_r(\bm{s})=\sum_{i = 1}^{\hat{N}}\hat{f}_i(\bm{s})$, with estimates of the expected number and density of ACs in the $j$th cell given by $\widehat{E}_r(\delta_{\cdot j})=\int_{\mathcal{C}_j}\widehat{D}_r(\bm{s}) \, d\bm{s}$ and $\widehat{D}_{rj}=\int_{\mathcal{C}_j}\widehat{D}_r(\bm{s})\, d\bm{s}/a$ respectively. 

%Again, ML and Bayesian SCR methods for estimating the AC PDFs and hence the realised AC density surface, are well documented, for example in Section 4.3 of \cite{Borchers+Efford:08} for ML methods and the section ``Estimating derived parameters'' on page 3238 of \cite{Royle+al:09b} for Bayesian methods. Given spatial capture histories, both approaches estimate the locations of ACs. While ACs are points, there is always uncertainty associated with estimating their locations, so that SCR estimates of AC locations are AC PDFs, not points. Note that we can obtain AC PDFs for undetected animals because we know their capture histories -- namely no capture at every detector -- and these will be the same AC PDF unless there are individual-level covariates that affect detection probability estimates (a complication that we ignore). Separating out the estimated AC PDFs for $n$ detected animals, $\hat{f}_i(\bm{s}),\ldots,\hat{f}_n(\bm{s})$, from those for each of $\hat{N}-n$ undetected individuals, $\hat{f}_u(\bm{s})$, we obtain $\widehat{D}_r(\bm{s}) = \sum_{i = 1}^n \hat{f}_i(\bm{s}) + (\widehat{N} - n)\hat{f}_u(\bm{s})$.
...
Note that we can obtain AC PDFs for undetected animals because we know their capture histories -- namely no capture at every detector. We denote the AC PDF for these undetected individuals as $f_u(\bm{s})$. Note also that all undetected animals will have the same AC PDF because they all have the same capture history (unless there are individual-level covariates that affect detection probability estimates, a complication that we ignore here). 
...
\end{equation}
with $f_i(\bm{s})$, $f_u(\bm{s})$ and $N$ estimated by SCR. It is this surface, usually plotted after discretising the survey region into cells, that many publications (including those listed in the Introduction) interpret as a density surface for AC or animal locations. While perhaps done for brevity, referring to the realised AC density surface as an estimated density of {\it animals} makes the additional error of ignoring animal movement around ACs. An AC PDF captures only uncertainty about where the AC is located. It is not equivalent to an estimate of the animal's home range. 

% Note that we can obtain AC PDFs for undetected animals because we know their capture histories -- namely no capture at every detector -- and these will be the same AC PDF unless there are individual-level covariates that affect detection probability estimates (a complication that we ignore). Separating out the estimates of the AC PDFs for $n$ detected animals, $\hat{f}_i(\bm{s}),\ldots,\hat{f}_n(\bm{s})$, from those for each of $\hat{N}-n$ undetected individuals, $\hat{f}_u(\bm{s})$, we obtain $\widehat{D}_r(\bm{s}) = \sum_{i = 1}^n \hat{f}_i(\bm{s}) + (\widehat{N} - n)\hat{f}_u(\bm{s})$. It is this surface, usually plotted after discretising the survey region into cells, that many publications (including those listed in the Introduction) interpret as a density surface for AC or animal locations. While perhaps done for brevity, referring to the realised AC density surface as an estimated density of {\it animals} makes the additional error of ignoring animal movement around ACs. An AC PDF captures only uncertainty about where the AC is located. It is not equivalent to an estimate of the animal's home range and does not provide meaningful information on animal space use --  animals may be present at locations with realised AC densities close to zero.

\subsection{Uncertainty in estimated density surfaces} \label{uncertainty}

We can calculate and plot measures of uncertainty alongside AC density surface estimates using standard approaches.  For a frequentist analysis, options include pixel-specific standard errors, coefficients of variation, and upper- and lower-limits of confidence intervals. Alternatives for a Baysian analysis include posterior standard deviations and credible interval limits. Measures of uncertainty for the realised density within a pixel (i.e., number of activity centres in the pixel divided by its area) can be computed from the MCMC samples obtained from a Bayesian analysis. We provide further detail in Supplementary Material A.
...
For most surveys, the realised AC density surfaces produced from an SCR model cannot be interpreted as density maps in any meaningful biological or ecological way, because they depend heavily on factors related to the detection process that have nothing to do with the underlying AC intensity. 
...
\subsection{Uncertainty in estimated AC density surfaces}

Here we highlight two findings from the plots of uncertainty in expected and realised AC density surface estimates in Supplementary Material A. First, uncertainty of realised AC density surface estimates are sensitive to the pixel size selected by the analyst: we obtain higher relative precision when estimating the number of ACs in a large area than a small one, because obtaining high precision for small areas requires accurate estimates of AC locations. This complicates the reporting of uncertainty for realised AC density surface estimates, because pixel size is often selected arbitrarily. By contrast, uncertainties of expected AC density estimates are not sensitive to pixel size.

Second coefficients of variation (standard deviation of the posterior distribution divided by the mean) for expected AC density surface were far smaller than for realised AC density. This is because expected AC density relates to the mean of a distribution (the expected number of activity centres in the pixel, divided by the area of the pixel), whereas realised AC density relates to a random variable drawn from that distribution. We can estimate the mean of a distribution more precisely than predicting an outcome from that distribution, because the latter requires that we account for the variation of a single observation in addition to uncertainty in estimating the mean.

In summary, we highlight the complications of reporting uncertainty for realised AC density should made with care: authors should select and report a meaningful pixel size when reporting uncertainty, and clearly communicate that uncertainties relate to realisations of random variables, rather than their expectations.
...
\subsection{Ecologically interpretable density estimates}
