...
\end{equation}
with $f_i(\bm{s})$, $f_u(\bm{s})$ and $N$ estimated by SCR. This function is usually plotted after discretising the survey region into cells, with the estimated density in cell $j$ being $\widehat{D}_{rj}=\int_{\mathcal{C}_j}\widehat{D}_r(\bm{s})\, d\bm{s}/a$. It is this surface that many publications (including those listed in the Introduction) interpret as a density surface for AC or animal locations. While perhaps done for brevity, referring to the realised AC density surface as an estimated density of {\it animals} makes the additional error of ignoring animal movement around ACs. An AC PDF captures only uncertainty about where the AC is located. It is not equivalent to an estimate of the animal's home range. 
...
\subsection{Communcating uncertainty in estimated density}

\textcolor{red}{Communicating uncertainty in estimated density surfaces is one important issue we have neglected to mention so far for brevity. We recommend including plots of expected density surface estimates with plots of standard measures of estimate uncertainty. For a frequentist analysis, options include pixel-specific standard errors, coefficients of variation, and upper- and lower-limits of confidence intervals. Alternatives for a Baysian analysis include posterior standard deviations and credible interval limits. In Supplementary Materials A, we include upper- and lower-limits of credible intervals alongside the expected density surface estimates from our Bayesian analysis.}

\todo[inline]{dlb: I removed ``We  are  not  aware  of  any  publications  that  plot  uncertainty  in  realised  density  surfaceestimates,  however'' from the start of the next paragraph. Seemed unnecessary.}

\textcolor{red}{Measures of uncertainty for the realised density within a pixel (i.e., number of activity centres in the pixel divided by its area) can be computed from the MCMC samples obtained from a Bayesian analysis. In our experience, relative precision of a realised density estimate calculated in this way is sensitive to the size of the pixel selected by the analyst: we obtain higher relative precision when estimating the number of activity centres in a large area than a small one, because obtaining high precision for small areas requires accurate estimates of activity centre locations.}

\todo[inline]{I think this next paragraph would be better in the appendix.}
\textcolor{red}{What we mean by ``uncertainty'' can also depend on whether we are considering an expected or realised density surface. We obtain an expected density surface estimate by estimating the $\beta$ parameters. Under a frequentist paradigm, precision of a pixel's expected density surface estimator depends on its variance over all possible realisations of the data, and its bias is the difference between the mean of its distribution and the true parameter values. However, a realised density surface estimate aims to capture patterns specific to a single realisation of the activity centres, and there is no ``true'' realised density parameter that applies across all realisations of the activity centres. The ``precision'' of a pixel's realised density surface estimate relates to how close our estimate is to the true realised density for a single realisation, rather than the variance of the estimator over all possible realisations. It might be useful to adopt language relating to prediction when we describe uncertainty for a pixel's realised density estimate, because we are assessing how well we are able to predict the outcome of a single random variable (the number of activity centres in the pixel, divided by its area) rather than how well we are able to estimate a model parameter. Confidence intervals and prediction intervals are designed to achieve different goals, so we shouldn't expect estimation error for expected densities to align with prediction error for realised densities. In summary, we highlight that any interpretations of uncertainty for realised density should made with care: authors should select and report a meaningful pixel size to report uncertainty for, and be clear to define what they mean by ``uncertainty'' in such cases.}
