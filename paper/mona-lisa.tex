...
%true) ‘realised density surface’ does not exist (except perhaps as a scatter of Dirac distributions in the plane). Any surface purporting to be the ‘realised AC density’ is something else, and I dispute the idea that the sum-of-pdfs is an estimate of the realised AC point pattern (p7). Even with infinite sampling intensity it fails to resolve into a point pattern on the periphery. Conversely, there is no ‘true’ realised AC density surface until an algorithm is applied to infer a density surface from the point pattern. A better name is needed for the predominant mis-usage. Clearly some people think that is a valid ‘density’, so it should be defined (a derived quantity that is the sum of individual inferred pdfs for locations of detected animals and scaled, model-based pdf for location of an undetected animal, with no interpretation in terms of the underlying process?).
