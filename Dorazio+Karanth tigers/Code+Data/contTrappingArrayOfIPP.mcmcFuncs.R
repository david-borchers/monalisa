...
fit.contTrappingArrayOfIPP = function(niter=100, niterInterval=10, sgrid, indForNA, traploc, y, X, W, T,  adaptiveMHoutput=TRUE) {
...
piZeroLambda = function(alpha, sigma, beta, X, sgrid.cellsize, W, T, distmat) {  
  lambda = exp(X %*% beta)
  ...
  Tmat = matrix(rep(T, each=sgrid.ncell), ncol=K)
...
logDensity.beta = function(beta, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma) {
  logDensity.prior = sum(dnorm(beta, mean=beta.mu, sd=beta.sigma, log=TRUE))
  logDensity = -Lambda(beta, X, sgrid.cellsize) + n0*log(piZeroLambda(alpha, sigma, beta, X, sgrid.cellsize, W, T, distmat)) + sum(X.obs%*%beta)
  logDensity + logDensity.prior
}


neglogDensity.beta = function(beta, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma) {
   (-1)*logDensity.beta(beta, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma)
}

negGrad.beta = function(beta, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma) {
  lambda = exp(X %*% beta)
  sgrid.ncell = dim(distmat)[1]
  K = dim(distmat)[2]
  logPsi = W %*% alpha
...
logDensity.alphaANDlogsigma = function(param, beta, y, X, sgrid.cellsize, W, T, distmat, n0, distmat.obs, alpha.mu, alpha.sigma, sigma.nu, sigma.scale) {
    sigma = exp(param[1])
    alpha = param[-1]
    ...
    logDensity.prior = logDensity.alphaprior + logDensity.sigmaprior + log(sigma)
    logPsi = W %*% alpha
    n = dim(y)[1]
    K = dim(distmat)[2]
    logPsiMat = matrix(rep(logPsi, each=n), ncol=K)
    logPhiMat = logPsiMat - 0.5*(distmat.obs/sigma)^2
    Tmat = matrix(rep(T, each=n), ncol=K)
    PhiMat = exp(logPhiMat) * Tmat
    tempSum = sum(y*logPhiMat - T*exp(logPhiMat))
    logDensity = tempSum + n0*log(piZeroLambda(alpha, sigma, beta, X, sgrid.cellsize, W, T, distmat))
    logDensity + logDensity.prior
}

neglogDensity.alphaANDlogsigma = function(param, beta, y, X, sgrid.cellsize, W, T, distmat, n0, distmat.obs, alpha.mu, alpha.sigma, sigma.nu, sigma.scale) {
   (-1)*logDensity.alphaANDlogsigma(param, beta, y, X, sgrid.cellsize, W, T, distmat, n0, distmat.obs, alpha.mu, alpha.sigma, sigma.nu, sigma.scale)
}



## this conditional is used for predicting locations of n0 individuals not observed at any of K traps
logDensity.spred = function(s, Xloc, alpha, sigma, beta, X.pred, W, T) {
...
logDensity.s = function(s, yvec, Xloc, alpha, sigma, beta, X.obs, W, T) {
    logLambda = sum(X.obs*beta)
    smat = matrix(s, nrow=1)
    distVec = as.vector(rdist(Xloc, smat))
    logOfPhi = W %*% alpha - 0.5*(distVec/sigma)^2
    Phi = T*exp(logOfPhi)
    retVal = logLambda + sum(yvec*logOfPhi) - sum(Phi)
    retVal
}

neglogDensity.s = function(s, yvec, Xloc, alpha, sigma, beta, X.obs, W, T) {
    (-1)*logDensity.s(s, yvec, Xloc, alpha, sigma, beta, X.obs, W, T)
}

negGrad.s = function(s, yvec, Xloc, alpha, sigma, beta, X.obs, W, T) {
...
negHess.s = function(s, yvec, Xloc, alpha, sigma, beta, X.obs, W, T) {
    ## this code assumes that X.obs does not include s as a regressor
    smat = matrix(s, nrow=1)
    distVec = as.vector(rdist(Xloc, smat))
    logOfPhi = W %*% alpha - 0.5*(distVec/sigma)^2
...
alpha = rep(0, ncol(W))
alpha0 = log( 10 * mean(apply(y,2,mean)/(T)) )
alpha[1] = alpha0
...
cat(c('n0','sigma', alpha.names, beta.names), sep=',', file='mc.csv')
...
while(continueGibbs) {
  ...
  if (floor(draw/batchsize)*batchsize == draw) {
    ...
    ##  draw n0
  n0 = rpois(1, piZeroLambda(alpha, sigma, beta, X, sgrid.cellsize, W, T, distmat))
  

  ##  draw loc (= activity center of each observed individual)
  for (i in 1:n) {
      ## ... find mode and hessian of unnormalized conditional density function
      fit = optim(par=loc[i,], fn=neglogDensity.s, gr=negGrad.s, method='BFGS', hessian=FALSE, yvec=y[i,], Xloc=traploc, alpha=alpha, sigma=sigma, beta=beta, X.obs=X.obs[i,], W=W, T=T)

      fit.gradient = negGrad.s(fit$par, yvec=y[i,], Xloc=traploc, alpha=alpha, sigma=sigma, beta=beta, X.obs=X.obs[i,], W=W, T=T)
      fit.hessian = negHess.s(fit$par, yvec=y[i,], Xloc=traploc, alpha=alpha, sigma=sigma, beta=beta, X.obs=X.obs[i,], W=W, T=T)
      ...
      if (fit$conv==0 & sum(abs(fit.gradient))<0.0001*abs(fit$value) & all(diag(fit.hessian)>0) )  {  # test validity of MH proposal
          ...
          if (xyInRaster(loc.cand, sgrid.xylimits)) {
              logL.loc.cand = logDensity.s(loc.cand, yvec=y[i,], Xloc=traploc, alpha=alpha, sigma=sigma, beta=beta, X.obs=X.obs.cand, W=W, T=T)
              logL.loc = logDensity.s(loc[i,], yvec=y[i,], Xloc=traploc, alpha=alpha, sigma=sigma, beta=beta, X.obs=X.obs[i,], W=W, T=T)
              ...
              if (logR >= 0 | runif(1,0,1) <= exp(logR)) {
                  ...
                  if (xyInRaster(loc.cand2, sgrid.xylimits)) {
                      logL.loc.cand2 = logDensity.s(loc.cand2, yvec=y[i,], Xloc=traploc, alpha=alpha, sigma=sigma, beta=beta, X.obs=X.obs.cand2, W=W, T=T)
  ...
  ## draw beta
  ## ... find mode and hessian of unnormalized conditional density function
  fit = optim(par=beta, fn=neglogDensity.beta, gr=negGrad.beta, method='BFGS', hessian=TRUE, alpha=alpha, sigma=sigma, X=X, sgrid.cellsize=sgrid.cellsize, W=W, T=T, distmat=distmat, n0=n0, X.obs=X.obs, beta.mu=beta.mu, beta.sigma=beta.sigma)

  fit.gradient = negGrad.beta(fit$par, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma)
  ...
  if (fit$conv==0 & sum(abs(fit.gradient))<0.0001*abs(fit$value) & all(diag(fit.hessian)>0) )  {  # test validity of proposal
      ...
      logL.beta.cand = logDensity.beta(beta.cand, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma)
      logL.beta = logDensity.beta(beta, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma)
      ...
      if (logR >= 0 | runif(1,0,1) <= exp(logR)) {
          ...
          logL.beta.cand2 = logDensity.beta(beta.cand2, alpha, sigma, X, sgrid.cellsize, W, T, distmat, n0, X.obs, beta.mu, beta.sigma)
  ...
  ## draw alpha and sigma
  ## ... find mode and hessian of unnormalized conditional density function
  param = c(log(sigma), alpha)
  fit = optim(par=param, fn=neglogDensity.alphaANDlogsigma, method='BFGS', hessian=TRUE, beta=beta, y=y, X=X, sgrid.cellsize=sgrid.cellsize, W=W, T=T, distmat=distmat, n0=n0, distmat.obs=distmat.obs, alpha.mu=alpha.mu, alpha.sigma=alpha.sigma, sigma.nu=sigma.nu, sigma.scale=sigma.scale)
  fit.hessian = fit$hessian
  ...
  if (fit$conv==0 & all(diag(fit.hessian)>0) )  {  # test validity of proposal
      ...
      logL.param.cand = logDensity.alphaANDlogsigma(param.cand, beta, y, X, sgrid.cellsize, W, T, distmat, n0, distmat.obs, alpha.mu, alpha.sigma, sigma.nu, sigma.scale)
      logL.param = logDensity.alphaANDlogsigma(param, beta, y, X, sgrid.cellsize, W, T, distmat, n0, distmat.obs, alpha.mu, alpha.sigma, sigma.nu, sigma.scale)
      ...
      if (logR >= 0 | runif(1,0,1) <= exp(logR)) {
          ...
          logL.param.cand2 = logDensity.alphaANDlogsigma(param.cand2, beta, y, X, sgrid.cellsize, W, T, distmat, n0, distmat.obs, alpha.mu, alpha.sigma, sigma.nu, sigma.scale)
  ...
  cat(c(n0, sigma, alpha, beta), sep=',', file='mc.csv', append=TRUE)
  cat(CR, file='mc.csv', append=TRUE)
