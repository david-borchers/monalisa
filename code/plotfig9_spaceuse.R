...
predicted_densities_all <- fig67_results_100sim %>% purrr::map_depth(2, "predicted_densities") %>% map_df(bind_rows)
...
estimated_sigma <- fig67_results_100sim %>% purrr::map_depth(2, "estimated_sigma") %>% map_df(bind_rows)
estimated_sigma <- estimated_sigma %>% mutate(sim_id = rep(1:100, each = 12))
# REMOVE COMMENT IF YOU WANT just one sim (first one)
