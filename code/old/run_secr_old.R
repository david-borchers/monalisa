...
run_secr <- function(simulated_points,
  ...
  for(i in noccasions){
    ...
    if (str_detect(secr.fitformula, "1")) { 
      tmp <- fx.total(cfit)
      expnumber_ac <- covariates(tmp)$D.sum
      prob_ac <- rep(NA, length(expnumber_ac))
      prob_ac_seenonly <- rep(NA, length(expnumber_ac))
      expnumber_ac_seenonly <- covariates(tmp)$D.fx
