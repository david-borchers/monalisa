...
# Firstly, each row in this data frame will be copied three times (due to the nature of the data frame they were extracted from) - so our first job is to remove these duplicated rows. There are 7500 rows in the data frame. We use this information and begin by generating a sequence that gives the indices of the 2500 rows that we want (so that there are no longer any duplicates)
...
dgood = mona.densities[,"Dgood"]

ch7c.sample = run.MCMC.inhom(data.ch7c, M=9000, covariate=dgood, n.iter=100000)
